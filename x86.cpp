...
void x86Program::handle_binop(llvm::BasicBlock::const_iterator it) {
    llvm::BinaryOperator const &op = llvm::cast<llvm::BinaryOperator>(*it);
    //no need for handle if no use later
    ...
    x86Destination *dst = acquire_slot(op);
    x86Source *src = nullptr;

    //determining the proper source and inserting instructions
    if (llvm::isa<llvm::ConstantInt>(*lhs)) {
         src = new x86Immediate(llvm::cast<llvm::ConstantInt>(*lhs));
         if (llvm::isa<llvm::ConstantInt>(*rhs)) {
            //moving the constant into register for use
            insert_instruction(new x86SrcDstInstruction("movq", new x86Immediate(llvm::cast<llvm::ConstantInt>(*rhs)), dst));
         } else {
            insert_instruction(new x86SrcDstInstruction("movq", query_slot(*rhs), dst));
         }
    } else {
        if (llvm::isa<llvm::ConstantInt>(*rhs)) {
            //moving the constant into register for use
            src = new x86Immediate(llvm::cast<llvm::ConstantInt>(*rhs));
        } else {
            src = query_slot(*rhs);
        }
        if (llvm::isa<llvm::ConstantInt>(*lhs)) {
            insert_instruction(new x86SrcDstInstruction("movq", new x86Immediate(llvm::cast<llvm::ConstantInt>(*lhs)), dst));
        } else {
            insert_instruction(new x86SrcDstInstruction("movq", query_slot(*lhs), dst));
        }
    }
    //based on op code, insert the proper operational instruction
    std::string opcode;
    ...
    switch(op.getOpcode()){
        ...
        case llvm::Instruction::Mul:
            ...
            break;
        default:
            llvm::errs() << "Opcode not found";
            break;
        
        
    }
    //insert final instruciton
    insert_instruction(new x86SrcDstInstruction(opcode,src,dst));
}


void x86Program::handle_cmp(llvm::BasicBlock::const_iterator it) {
    llvm::Instruction const &op = llvm::cast<llvm::Instruction>(*it);
    //getting the comparison operands
    llvm::Value *lhs = op.getOperand(0);       
    llvm::Value *rhs = op.getOperand(1);

    x86Source *lsrc = nullptr;
    x86Source *rsrc = nullptr;

    //initate to x86immediate if the type is constant, else get source via query slot
    if (llvm::isa<llvm::ConstantInt>(*lhs)) {
        lsrc = new x86Immediate(llvm::cast<llvm::ConstantInt>(*lhs));
    } else {
        lsrc = query_slot(*lhs);
    }
    //moving instruction to rax register
    insert_instruction(new x86SrcDstInstruction("movq",lsrc,new x86Register("rax")));
    if (llvm::isa<llvm::ConstantInt>(*rhs)) {
        rsrc = new x86Immediate(llvm::cast<llvm::ConstantInt>(*rhs));
    } else {
        rsrc = query_slot(*rhs);
    }
    //final insertion
    insert_instruction(new x86SrcDstInstruction("cmp", rsrc, new x86Register("rax")));
}
